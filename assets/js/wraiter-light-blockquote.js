if (typeof jQuery !== 'undefined') {
	...
	jQuery(window).on('elementor/init', function () {
		elementor.channels.editor.on('wraiterlight:blockquote:request', function (controlView) {
			const WRAITER_LIGHT_Blockquotecontent_Generator = {
				controls: '',
				field: '',
				settings: '',
				genButton: '',
				...
				init(controlView) {
					...
					this.field = elementor.getPanelView().currentPageView.getControlViewByName('blockquote_content');
					this.settings = controlView.container.settings.attributes;
					this.genButton = this.controls.find('.elementor-button-default[data-event="wraiterlight:blockquote:request"]');
			...
			WRAITER_LIGHT_Blockquotecontent_Generator.init(controlView);
		});
	});
}
