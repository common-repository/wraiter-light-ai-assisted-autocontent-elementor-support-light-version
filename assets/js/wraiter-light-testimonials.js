if (typeof jQuery !== 'undefined') {
	...
	jQuery(window).on('elementor/init', function () {
		elementor.channels.editor.on('wraiterlight:testimonial:request', function (controlView) {
			const WRAITER_LIGHT_Testimonialcontent_Generator = {
				controls: '',
				field: '',
				settings: '',
				genButton: '',
				...
				init(controlView) {
					...
					this.field = elementor.getPanelView().currentPageView.getControlViewByName('testimonial_content');
					this.settings = controlView.container.settings.attributes;
					this.genButton = this.controls.find('.elementor-button-default[data-event="wraiterlight:testimonial:request"]');
				...
				success_request(response) {
					if (!response.success) {
						...
						this.field.setSettingsModel(response.data[0]);
						this.field.applySavedValue();
					}
				},
			};

			WRAITER_LIGHT_Testimonialcontent_Generator.init(controlView);
		});
	});
}
