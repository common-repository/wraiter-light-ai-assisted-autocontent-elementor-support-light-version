if (typeof jQuery !== 'undefined') {
	...
	jQuery(window).on('elementor/init', function () {
		elementor.channels.editor.on('wraiterlight:htmlcontent:request', function (controlView) {
			const WRAITER_LIGHT_Htmlcontent_Generator = {
				controls: '',
				field: '',
				settings: '',
				genButton: '',
				...
				init(controlView) {
					...
					this.field = elementor.getPanelView().currentPageView.getControlViewByName('html');
					this.settings = controlView.container.settings.attributes;
					this.genButton = this.controls.find('.elementor-button-default[data-event="wraiterlight:htmlcontent:request"]');
			...
			WRAITER_LIGHT_Htmlcontent_Generator.init(controlView);
		});
	});
}
